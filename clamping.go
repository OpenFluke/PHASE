...
// ManualClampAll clamps all neuron values, biases, weights, and (for LSTM) cell states
// to the range [minVal, maxVal] in a single loop.
func (bp *Phase) ManualClampAll(minVal, maxVal float64) {
	if minVal > maxVal {
		fmt.Printf("Warning: minVal > maxVal. Swapping them.\n")
		minVal, maxVal = maxVal, minVal
	}
	for _, neuron := range bp.Neurons {
		...
		// Clamp CellState if neuron is LSTM
		if neuron.Type == "lstm" {
			if math.IsNaN(neuron.CellState) || math.IsInf(neuron.CellState, 0) {
				neuron.CellState = 0
			} else if neuron.CellState > maxVal {
